...
class MemcacheManager(object):
        ...
        try:
            if CAN_USE_MEMCACHE.value:
                ...
                if size > MEMCACHE_MAX:
                    logging.error('Contents of key %s is too big. Not saved in memcache' % key)
    ...
    @classmethod
    def delete_multi(cls, key_list, namespace=None, override_readonly=True):
        """Deletes a list of items from memcache if memcache is enabled."""
        assert override_readonly or not cls._IS_READONLY
        if CAN_USE_MEMCACHE.value:
...
class Student(BaseEntity):
    ...
    @classmethod
    def set_paid_status_for_current(cls, has_paid):
