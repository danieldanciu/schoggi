...
class DashboardHandler(
    ...
    post_actions = [
        'create_or_edit_settings', 'add_unit', 'add_appendix',
    ...
    def render_course_outline_to_html(self, course):
        ...
        for unit in course.get_units():
            ...
            elif unit.type == verify.UNIT_TYPE_APPENDIX:
                lines.add_child(self._render_unit_li(course, unit))
            else:
    ...
    def get_outline(self):
        ...
        current_course = courses.Course(self)
        course_info.append('Schema Version: %s' % current_course.version)
        ...
        if self.app_context.is_editable_fs():
            ...
            all_units = current_course.get_units()
            if any([(unit.type == verify.UNIT_TYPE_UNIT or unit.type == verify.UNIT_TYPE_APPENDIX) for unit in all_units]):
                outline_actions.append({
                    'id': 'add_lesson',
                    'caption': 'Add Lesson',
            ...
            outline_actions.append({
                'id': 'add_unit',
                'caption': 'Add Appendix',
                'action': self.get_action_url('add_appendix'),
                'xsrf_token': self.create_xsrf_token('add_appendix')})
            ...
            if not current_course.get_units():
                outline_actions.append({
                    'id': 'import_course',
                    'caption': 'Import',
        ...
        sections = [
                ...
                'children': pages_info},
            {
                ...
                'pre': self.render_course_outline_to_html(current_course)}]

        if current_course.version == courses.COURSE_MODEL_VERSION_1_2:
            sections.append({
                'title': 'Data Files',
    ...
    def list_questions(self, all_questions, all_question_groups, location_maps):
        ...
        output = safe_dom.NodeList().append(
            safe_dom.Element(
                'a', className='gcb-button gcb-pull-right',
                href='dashboard?action=import_sa_question_group'
            ).add_text('Import Q&A file')
        ).append(
            safe_dom.Element(
                'a', className='gcb-button gcb-pull-right',
    ...
    def get_assets_questions(self, items, tab, all_paths):
        ...
        return all_questions, all_question_groups, locations
