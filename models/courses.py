...
MEMCACHE_CHUNK_SIZE = models.MEMCACHE_MAX - 100 # additional buffer for pickle
...
class AbstractCachedObject(object):
    ...
    def _make_key(cls):
        ...
        # deployment.
        return 'course:model:pickle_chunked:%s:%s' % (
    ...
    def load(cls, app_context):
        ...
        try:
            chunk_keys = MemcacheManager.get(
                cls._make_key(),
            ...
            for chunk_key in chunk_keys:
                ...
                chunk = MemcacheManager.get(chunk_key)
                if chunk is None:
                    return None
                chunks.append(chunk)
    ...
    def delete(cls, app_context, override_readonly=False):
        ...
        key = cls._make_key()
        chunk_keys = MemcacheManager.get(key, namespace=app_context.get_namespace_name())
        if chunk_keys is None:
            return False
    ...
    def save(cls, app_context, instance):
        ...
        chunk_keys = []
        for pos in range(0, binary_data_size, MEMCACHE_CHUNK_SIZE):
...
class CourseModel13(object):
    ...
    def update_lesson(self, lesson):
        ...
        existing_lesson.audio = lesson.audio
    ...
    def reorder_units(self, order_data):
        ...
        for unit_data in order_data:
            ...
            unit = self.find_unit_by_id(unit_id)
            assert unit
            if verify.UNIT_TYPE_UNIT != unit.type and verify.UNIT_TYPE_APPENDIX != unit.type:
                continue
    ...
    def get_component_locations(self):
        ...
        for unit in self.get_units():
            if unit.type == verify.UNIT_TYPE_ASSESSMENT:
                ...
                unit_lessons = self.get_lessons(unit.unit_id)
                for lesson in unit_lessons:
                    lesson_components = self.get_components(
                        unit.unit_id, lesson.lesson_id)
